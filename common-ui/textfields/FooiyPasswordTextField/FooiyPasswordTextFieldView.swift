...
class FooiyPasswordTextFieldView: UIView {
    ...
    public var isMasked: Bool = false {
        didSet {
            setState()
            textField.isMasked = self.isMasked
        }
    }
    ...
    public let textField: FooiyPasswordTextField = {
        let textField = FooiyPasswordTextField()
    ...
    let helperLabel: UILabel = {
        ...
        label.textColor = FooiyColors.P800
        label.font = FooiyFonts.Caption3
        return label
    }()
    ...
    private func setStackView() {
        ...
        helperLabel.snp.makeConstraints {
            $0.width.equalToSuperview().offset(-FooiyTextFieldView.Dimension.helperLabelHorizontalMargin*2)
        }
    ...
    private func setState() {
        ...
        textRelay
            ...
            .bind(onNext: { [weak self] isEmpty in
                if isEmpty {
                    self?.textField.maskingButton.tintColor = FooiyColors.G200
                    self?.textFieldLine.backgroundColor = FooiyColors.G200
                } else {
                    self?.textField.maskingButton.tintColor = FooiyColors.G400
                    self?.textFieldLine.backgroundColor = FooiyColors.G400
                }
            }).disposed(by: disposeBag)
